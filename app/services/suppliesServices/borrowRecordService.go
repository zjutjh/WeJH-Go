...
func GetBorrowRecord(campus uint8, status int, studentID string) ([]models.BorrowRecord, error) {
	...
	var result *gorm.DB
	if status == 1 {
		result = ndb.Pick().Where(models.BorrowRecord{
			Campus:    campus,
			Status:    1,
			StudentID: studentID,
...
func CreateBorrowRecord(borrowRecord models.BorrowRecord) error {
	...
	result := ndb.Pick().Create(&borrowRecord)
	return result.Error
}
...
func GetBorrowRecordsByBorrowIDs(borrowIDs []int) ([]models.BorrowRecord, error) {
	...
	result := ndb.Pick().Where("id IN ?", borrowIDs).Find(&borrowRecords)
	if result.Error != nil {
...
func DeleteRecord(RecordID int) error {
	result := ndb.Pick().Delete(models.BorrowRecord{ID: RecordID})
	return result.Error
}
...
func GetRecordByAdmin(pageNum, pageSize, status, choice, id int, campus uint8, studentID, name, spec string) ([]models.BorrowRecord, *int64, error) {
	...
	var num int64
	query := ndb.Pick().Table("borrow_records").
...
func PassBorrow(id int, sid int, num uint) error {
	result := ndb.Pick().Model(models.BorrowRecord{}).Where(models.BorrowRecord{ID: id}).Updates(models.BorrowRecord{Status: 3, BorrowTime: time.Now()})
	if result.Error != nil {
		return result.Error
	}
	result = ndb.Pick().Model(models.Supplies{}).Unscoped().Where(models.Supplies{ID: sid}).Updates(map[string]interface{}{"stock": gorm.Expr("stock - ?", num), "borrowed": gorm.Expr("borrowed + ?", num)})
	return result.Error
}

func PassBorrows(ids []int) error {
	result := ndb.Pick().Model(models.BorrowRecord{}).Where("id IN ?", ids).Updates(map[string]interface{}{"status": 3, "borrow_time": time.Now()})
	if result.Error != nil {
		return result.Error
	}
	var records []models.BorrowRecord
	result = ndb.Pick().Where("id IN ?", ids).Find(&records)
	if result.Error != nil {
		return result.Error
	}
	for i := range records {
		result = ndb.Pick().Model(models.Supplies{}).Unscoped().Where(models.Supplies{ID: records[i].SuppliesID}).Updates(map[string]interface{}{"stock": gorm.Expr("stock - ?", records[i].Count), "borrowed": gorm.Expr("borrowed + ?", records[i].Count)})
		if result.Error != nil {
			return result.Error
		}
	}
...
func RejectBorrow(id int) error {
	result := ndb.Pick().Model(models.BorrowRecord{}).Where(models.BorrowRecord{ID: id}).Update("status", 2)
	return result.Error
}

func RejectBorrows(id []int) error {
	result := ndb.Pick().Model(models.BorrowRecord{}).Where("id IN ?", id).Update("status", 2)
	return result.Error
}

func CancelRejectBorrow(id int) error {
	result := ndb.Pick().Model(models.BorrowRecord{}).Where(models.BorrowRecord{ID: id}).Update("status", 1)
	return result.Error
}

func ReturnBorrow(id int, sid int, num uint) error {
	result := ndb.Pick().Model(models.BorrowRecord{}).Where(models.BorrowRecord{ID: id}).Updates(models.BorrowRecord{Status: 4, ReturnTime: time.Now()})
	if result.Error != nil {
		return result.Error
	}
	var supplies models.Supplies
	result = ndb.Pick().Where(models.Supplies{ID: sid}).Unscoped().First(&supplies)
	if result.Error != nil {
		return result.Error
	}
	if supplies.Kind == "正装" {
		result = ndb.Pick().Model(models.Supplies{}).Where(models.Supplies{ID: sid}).Unscoped().Updates(map[string]interface{}{"stock": gorm.Expr("stock + ?", num), "borrowed": gorm.Expr("borrowed - ?", num)})
	}
	return result.Error
}

func ReturnBorrows(ids []int) error {
	result := ndb.Pick().Model(models.BorrowRecord{}).Where("id IN ?", ids).Updates(map[string]interface{}{"status": 4, "return_time": time.Now()})
	if result.Error != nil {
		return result.Error
	}
	var records []models.BorrowRecord
	...
	for i := range records {
		...
		result := ndb.Pick().Where(models.Supplies{ID: records[i].SuppliesID}).Unscoped().First(&supply)
		if result.Error != nil {
			return result.Error
		}
	...
	for i := range supplies {
		if supplies[i].Kind == "正装" {
			result = ndb.Pick().Model(models.Supplies{}).Where(models.Supplies{ID: supplies[i].ID}).Unscoped().Updates(map[string]interface{}{"stock": gorm.Expr("stock + ?", records[i].Count), "borrowed": gorm.Expr("borrowed - ?", records[i].Count)})
			if result.Error != nil {
				return result.Error
			}
		}
...
func CancelBorrow(id int, sid int, num uint) error {
	...
	if supplies.Kind == "正装" {
		result = ndb.Pick().Model(models.BorrowRecord{}).Where(models.BorrowRecord{ID: id}).Updates(map[string]interface{}{"status": 1, "borrow_time": nil})
		if result.Error != nil {
			return result.Error
		}
		result = ndb.Pick().Model(models.Supplies{}).Where(models.Supplies{ID: sid}).Unscoped().Updates(map[string]interface{}{"stock": gorm.Expr("stock + ?", num), "borrowed": gorm.Expr("borrowed - ?", num)})
	} else {
		result = ndb.Pick().Delete(models.BorrowRecord{ID: id})
	}
	return result.Error
}
...
func CancelBorrows(id []int) error {
	var records []models.BorrowRecord
	result := ndb.Pick().Where("id IN ?", id).Find(&records)
	if result.Error != nil {
		return result.Error
	}
	var supplies []models.Supplies
	...
	for i := range supplies {
		if supplies[i].Kind == "正装" {
			result = ndb.Pick().Model(models.BorrowRecord{}).Where("id IN ?", id).Updates(map[string]interface{}{"status": 1, "borrow_time": nil})
			if result.Error != nil {
				return result.Error
			}
			result = ndb.Pick().Model(models.Supplies{}).Where(models.Supplies{ID: supplies[i].ID}).Unscoped().Updates(map[string]interface{}{"stock": gorm.Expr("stock + ?", records[i].Count), "borrowed": gorm.Expr("borrowed - ?", records[i].Count)})
		} else {
			result = ndb.Pick().Delete(models.BorrowRecord{ID: records[i].ID})
		}
	}
	return result.Error
}
...
func CancelReturnBorrow(id int, sid int, num uint) error {
	result := ndb.Pick().Model(models.BorrowRecord{}).Where(models.BorrowRecord{ID: id}).Updates(map[string]interface{}{"status": 3, "return_time": nil})
	if result.Error != nil {
		return result.Error
	}
	var supplies models.Supplies
...
func UpdateRecord(id int, name, gender, college, dormitory, contact string, suppliesid int, count uint) error {
	...
	result := ndb.Pick().Model(models.BorrowRecord{}).Where(models.BorrowRecord{ID: id}).Updates(models.BorrowRecord{
		Name:       name,
		Gender:     gender,
		College:    college,
		Dormitory:  dormitory,
...
func GetSuppliesID(campus uint8, organization string, kind string, name string, spec string) (int, error) {
	var supplies *models.Supplies
	result := ndb.Pick().Order("id DESC").Where(models.Supplies{
		Kind:         kind,
		Name:         name,
		Spec:         spec,
		Campus:       campus,
...
func DeleteBorrowRecord(borrowRecord models.BorrowRecord) error {
	result := ndb.Pick().Model(models.BorrowRecord{}).Delete(&borrowRecord)
	return result.Error
}
